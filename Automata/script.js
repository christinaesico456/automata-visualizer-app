...
//   row-bot(y=410):             sB            sD
//
//  Arrows exactly as drawn:
//   start --1--> sA     (diagonal up-right)
//   start --0--> sB     (diagonal down-right)
//   sA    --1--> sC     (rightward top)
//   sA    --0--> sB     (downward, same x)        ← bidirectional with sB→sA
//   sB    --1--> sA     (upward, same x)           ← bidirectional
...
function nodeAlpha(id) {
  if (!BUILDUP.active && Object.keys(BUILDUP.nodeProgress).length === 0) return 1;
  return BUILDUP.nodeProgress[id] ?? 1;
}

function edgeProg(from, to) {
  if (!BUILDUP.active && Object.keys(BUILDUP.edgeProgress).length === 0) return 1;
  const key = `${from}→${to}`;
...
function angleBetween(aId, bId) {
  const a = DFA.positions[aId], b = DFA.positions[bId];
...
function straightEdgePts(from, to, offset = 0) {
  ...
  const a = DFA.positions[from], b = DFA.positions[to];
  return {
...
function getEdgeCurve(from, to) {
  ...
  // "from" arrow bows LEFT of direction; "to" arrow bows RIGHT of direction
  const CURVE_BOW = 42;
  const EDGE_OFF  = 16;
  const perp = angle + Math.PI / 2;
  ...
  const mx  = (sx + ex) / 2;
  const my  = (sy + ey) / 2;
  const cpx = mx + nx * CURVE_BOW;
  const cpy = my + ny * CURVE_BOW;
...
function drawEdge(e, isActive) {
  ...
  if (from === to) {
    ...
    if (buildGlow) { ctx.shadowColor = edgeColor; ctx.shadowBlur = 8; }
    ctx.beginPath();
  ...
  if (prog > 0.88) {
    ...
    // Label sits on the perpendicular-to-tangent side, offset outward
...
function drawNode(id, isActive) {
  ...
  const grad = ctx.createRadialGradient(
    p.x - NODE_R * 0.3, p.y - NODE_R * 0.3, 2, p.x, p.y, NODE_R
  );
  if (isActive) {
    if (s.isTrap) {
...
function applyStep(stepIdx) {
  ...
  particleT = 0; pulseT = 1;
  shipTrail = []; shipPos = null;
...
function loadInput() {
  ...
  if (!re.test(raw)) {
    alert(`Please enter a string containing only: ${DFA.alphabet.map((c) => `"${c}"`).join(", ")}`);
    return;
  }
  inputStr = raw;
  executionSteps = buildTrace(inputStr);
  resetAll();
  addLog(`Loaded: "${inputStr}" (${inputStr.length} symbol${inputStr.length !== 1 ? "s" : ""})`);
  applyReset();
}
function updateBtns() {
